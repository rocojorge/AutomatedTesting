...
def ConnectToSSH(hostname,username,password, port=22):
    if type(port) != int:
        print('Port must be an integer')
        return 'port not integer', 2
    if not port: port = 22
    try:
...
def ConnectToSCP(hostname,username,password, port=22):
    if type(port) != int:
        print('Port must be an integer')
        return 'port not integer', 2
    if not port: port = 22
    ...
    try:
        ...
        cliente.connect(hostname=hostname, #IFX_SCONF_DEVICE_ADDRESS
                        ...
                        auth_timeout=25,
                        port=port) #3
        SCP_Transport = scp.SCPClient(cliente.get_transport()) #4
        error = None
    except paramiko.AuthenticationException as error_paramiko:
            print(error_paramiko)
            error=1
            ...
            print(error_paramiko)
            return error_paramiko,error
    except scp.SCPException as error_SCP:
        print(error_SCP)
        return error_SCP, error
    except:
            print('Error Desconocido')
            return 'Error Desconocido',error
    return SCP_Transport,None #
...
def CreateFile(file_name, ruta_file):
    with open(ruta_file,'w') as archivo: archivo.write(file_name) #Questo per creare i file, ma, non lo useremo, solo per te per sapere di cosa si tratta
...
def main():
    ...
    session,error1 = ConnectToSSH( ifx_sconf.get_Device_Address(), ifx_sconf.get_Device_Username(), ifx_sconf.get_Device_Password() )
    if not check_root(session):
